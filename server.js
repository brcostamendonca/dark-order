...
var PRESENTATIONS_COLLECTION = "presentations";
...
mongodb.MongoClient.connect(process.env.MONGODB_URI || /*"mongodb://localhost:27017/test"*/ "mongodb+srv://Kasmo:RzDL6qJmN870LO9K@mongocluster-i8ckj.mongodb.net/test?retryWrites=true&w=majority" , function (err, client) {
  if (err) {
    console.log(err);
    process.exit(1);
  }
...
// PRESENTATION API ROUTES BELOW

/*  "/api/presentations"
 *    GET: finds all presentations
...
app.post("/api/presentations", function (req, res) {
  ...
  if (!req.body.title) {
    ...
    db.collection(PRESENTATIONS_COLLECTION).insertOne(newContact, function (err, doc) {
      if (err) {
        handleError(res, err.message, "Failed to create new presentation.");
      } else {
        res.status(201).json(doc.ops[0]);
      }
    });
 ...
 *    GET: find presentation by id
 *    PUT: update presentation by id
 *    DELETE: deletes presentation by id
 */

app.get("/api/presentations/:id", function (req, res) {
  db.collection(PRESENTATIONS_COLLECTION).findOne({ _id: new ObjectID(req.params.id) }, function (err, doc) {
    if (err) {
      handleError(res, err.message, "Failed to get presentation");
    } else {
      res.status(200).json(doc);
    }
  });
...
app.put("/api/presentations/:id", function (req, res) {
  ...
  db.collection(PRESENTATIONS_COLLECTION).updateOne({ _id: new ObjectID(req.params.id) }, { $set: updateDoc }, function (err, doc) {
    if (err) {
      handleError(res, err, "Failed to update presentation");
    } else {
      updateDoc._id = req.params.id;
      res.status(200).json(updateDoc);
    }
